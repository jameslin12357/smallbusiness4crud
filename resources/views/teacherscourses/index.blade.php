        ...
        <div class="row">
            <div class="col-md-3">
                <table class="table">
                    ...
                    <tr>
                        <th scope="row" class="active-tab"><a href="/teacherscourses">Teacherscourses</a></th>
                        <!--<td>Larry</td>-->
                        <!--<td>the Bird</td>-->
                        <!--<td>@twitter</td>-->
                    </tr>
                ...
                <table class="table">
                    ...
                    <tr>
                        ...
                        @if (Auth::user()->level === 1)
                            <th scope="col"><a href="/teacherscourses/create" class="btn btn-outline-secondary">Create</a></th>
                        @endif

                    </tr>
                    </thead>
                    <tbody>
                    @foreach ($teacherscourses as $teachercourse)
                        <tr>
                            <td>{{ $teachercourse->id }}</td>
                            <td>{{ $teachercourse->teacher_id }}</td>
                            <td>{{ $teachercourse->course_id }}</td>
                            <td>{{ $teachercourse->date_created }}</td>
                            <td>{{ $teachercourse->date_edited }}</td>
                            @if (Auth::user()->level === 1)
                                <td>
                                    <a href="/teacherscourses/{{ $teachercourse->id }}/edit" class="btn btn-outline-secondary">Edit</a>
                                </td>
                                <td>
                                    <form action="/teacherscourses/{{ $teachercourse->id }}" method="post">
                                        @csrf
                                        @method('DELETE')
